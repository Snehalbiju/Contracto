                                        ...
                                        <div class="chat-lists">
                                            <div class="tab-content" id="myTabContent">
                                                <div class="tab-pane fade show active" id="Open" role="tabpanel" aria-labelledby="Open-tab">
                                                    <!-- chat-list -->
                                                    <%
                                                        String selQry = "select * from tbl_work w inner join tbl_client cl on w.client_id=cl.client_id INNER JOIN tbl_workrequest wr ON w.work_id=wr.work_id where wr.contractor_id='" + session.getAttribute("c_id") + "' group by w.client_id";
                                                        ResultSet rs = con.selectCommand(selQry);
                                                        while (rs.next()) {
                                                    %>
                                                    <div class="chat-list">
                                                        <a class="d-flex align-items-center" href="Chat.jsp?cid=<%=rs.getString("client_id")%>&cname=<%=rs.getString("client_name")%>&cphoto=<%=rs.getString("client_photo")%>">
                                                            <div class="flex-shrink-0">
                                                                <img style="width: 32px; height:32px " class="rounded-circle"  src="../Assets/Files/ClientPhoto/<%=rs.getString("client_photo")%>" alt="">
                                                            </div>
                                                            <div class="flex-grow-1 ms-3">
                                                                <h6 class="text-black"><%=rs.getString("client_name")%>
                                                                <%
                                                                    String selQry1="select count(chat_id) as unread_msg from tbl_chat where chat_status=0 and chat_fromclient='"+rs.getString("client_id")+"' and chat_tocon='"+session.getAttribute("c_id")+"'";
                                            ...
                                            <div class="col-8">
                                                <div class="d-flex align-items-center">
                                                    <span class="chat-icon text-blue-blue-700"><i class="bi bi-caret-left-fill"></i></span>
                                                        ...
                                                        %>
                                                    <div class="flex-shrink-0">
                                                        <img class="rounded-circle" width="40" height="40" src="../Assets/Files/ClientPhoto/<%=request.getParameter("cphoto")%>" alt="">
                                                    </div>
                                                    <div class="flex-grow-1 ms-3 ">
